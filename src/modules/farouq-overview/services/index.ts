...
export const useFarouqOverviewService = () => {
  ...
  const syncFarouqLogs = async () => {
    try{
      setIsSyncing(true);
      const res = await api.get('/sync/connections/sphereops');
      toast.success('sphereops data synced successfully');
      setIsSyncing(false);
      return res.data;
    }
    catch (error: any) {
      const errorMessage = error.response?.data?.message || error.message || 'Error syncing sphereops data';
